...
// Loader + evaluation facade for the "v2" NNUE format: Full_Threats (60,720
...
//
// IMPORTANT SCOPE NOTE: this facade only supports *full* accumulator
// recomputation from a board position (evaluate()); it does not implement
// incremental per-move updates the way v1's VBoard does. Full_Threats
// features depend on piece-vs-piece attack relationships (not just a single
// piece's own square), so a single move can change many threat features at
// once -- unlike HalfKA-style features, a cheap incremental update isn't a
// small mechanical extension of v1's approach. Wiring this into VBoard's
// incremental make/unmake path is left as a deliberate follow-up.
...
namespace nnue_v2
{
    ...
    {
    public:
        using Model = NnueModelV2<NumFeatures, L1, NumPsqtBuckets, NumLsBuckets, L2, L3>;

    private:
        Model model;

    public:
        NnueEvalV2(Model &&_model) : model(std::move(_model)) {}

        explicit NnueEvalV2(const std::string &path)
