        ...
        for (int sq = 0; sq < 64; ++sq)
        {
            for (int pt = PAWN; pt <= KING; ++pt)
            {
                for (int c = 0; c < 2; ++c)
                {
                    ...
                    const int idx_w = nnue_v2::halfka::feature_index<WHITE>(ksq, piece_color, piece_type, sq);
                    const int idx_b = nnue_v2::halfka::feature_index<BLACK>(ksq, piece_color, piece_type, sq);

                    EXPECT_GE(idx_w, 0);
                    EXPECT_LT(idx_w, nnue_v2::halfka::NUM_REAL_FEATURES);
                    EXPECT_GE(idx_b, 0);
                    EXPECT_LT(idx_b, nnue_v2::halfka::NUM_REAL_FEATURES);
                }
            }
        }
    }
...
TEST(HalfkaV2HmEncoderTest, KingBucketOnlyDependsOnOrientedFile)
{
    ...
    for (int ksq = 0; ksq < 64; ++ksq)
    {
        const int o_ksq = nnue_v2::halfka::orient<WHITE>(ksq, ksq);
        EXPECT_GE(o_ksq % 8, 4);
        EXPECT_GE(nnue_v2::halfka::KingBuckets[o_ksq], 0);
    }
}
...
TEST(HalfkaV2HmEncoderTest, OwnAndEnemyKingNeverCollideForSameKsq)
{
    ...
    const int idx_own = nnue_v2::halfka::feature_index<WHITE>(ksq, WHITE, KING, ksq);
    const int idx_enemy = nnue_v2::halfka::feature_index<WHITE>(ksq, BLACK, KING, enemy_ksq);
