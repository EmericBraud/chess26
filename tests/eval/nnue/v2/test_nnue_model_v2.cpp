...
// bias [10, 20, 5, 8]). Note compute_l0 pairs acc[i] with acc[i+Half] (not
// adjacent elements): out[0] uses acc[0] & acc[2], out[1] uses acc[1] & acc[3].
//   L0 pairwise square (Half=2): out[0] = (10*5*127)>>7 = 49
//                                 out[1] = clamp((20*8*127)>>7, 0, 127) = 127
//   l0 = [49, 127, 49, 127]
//   L1 raw (weights [64,0,0,0] / [0,0,0,64]): raw[0] = 64*49 = 3136
//                                              raw[1] = 64*127 = 8128
//   skip_raw = raw[0] - raw[1] = -4992
//   x0 = 3136>>7 = 24; sqr0 = (24*24)>>7 = 4
//   x1 = 8128>>7 = 63; sqr1 = (63*63)>>7 = 31
//   l1_out = [4, 31, 24, 63]
//   L2 raw (weight all 64s, bias 0): raw = 64*(4+31+24+63) = 7808
//   x = 7808>>6 = 122; sqr = (122*122)>>7 = 116
//   l2_out = [116, 122]
//   l3_input = [4, 31, 24, 63, 116, 122]
//   output_raw (weights all 1s, bias 100) = 100 + 360 = 460
//   combined_raw = 460 + (-4992) = -4532
//   layerstack_final = trunc(-4532*75/128) = trunc(-2655.46875) = -2655
//   psqt_diff = 0
//   result = trunc((2*-2655 + 0) / 32) = trunc(-165.9375) = -165
using TestModel = NnueModelV2</*Features=*/4, /*Accum=*/4, /*PsqtBuckets=*/1, /*LsBuckets=*/1, /*L2=*/2, /*L3=*/1>;
...
namespace
{
    TestModel make_model()
    {
        ...
        acc_weights[0] = {5, 0, 0, 0}; // feature 0 nudges the accumulator so update_feature is observable
...
TEST(NnueModelV2Test, HandComputedForwardPassNoActiveFeatures)
{
    ...
    EXPECT_EQ(model.get_result<WHITE>(/*piece_count=*/32), -165);
    EXPECT_EQ(model.get_result<BLACK>(/*piece_count=*/32), -165);
}

TEST(NnueModelV2Test, UpdateFeatureChangesResult)
{
    TestModel model = make_model();
